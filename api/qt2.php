...
$app->get('/', function (Request $request, Response $response) { // สร้าง route ขึ้นมารองรับการเข้าถึง url
    $response->getBody()->write("Webservice Naylist"); // สร้างคำตอบกลับ
...
$app->post('/qtlist', function (Request $request, Response $response) {
    ...
    $sql = "SELECT B.SO_NO, B.SCHEDULE_ID, B.OU_CODE, B.BATCH_NO, B.STATUS, B.BATCH_ENTRY_DATE, B.TOTAL_ROLL, B.TOTAL_QTY,
                ...
                -- AND BATCH_ENTRY_DATE > SYSDATE - 52
                -- AND BATCH_ENTRY_DATE < SYSDATE - 51
...
$app->post('/qt', function (Request $request, Response $response) {
    ...
    $sql = "Select Distinct Dp.SNAGGING_RESISTANCE_METHOD,
	Dp.SNAGGING_CYCLE,
	Dp.SNAGGING_AS_RECEIVED_FACE,
	Dp.SNAGGING_AS_RECEIVED_BACK,
	Dp.SNAGGING_CLASSIFICATION_FACE,
	Dp.SNAGGING_CLASSIFICATION_BACK,
	Dp.SNAGGING_MORE_THAN2_FACE,
	Dp.SNAGGING_MORE_THAN2_BACK,
	Dp.SNAGGING_MORE_THAN5_FACE,
	Dp.SNAGGING_MORE_THAN5_BACK,
	Dp.SNAGGING_LESS_THAN5_FACE,
	Dp.SNAGGING_LESS_THAN5_BACK
	From DFQT_HEADER Dh,
			 DfQt_Phy2 Dp
	Where Dh.Ou_COde = Dp.Ou_COde
			And Dh.Qt_No = Dp.Qt_No
			And Dh.Ou_Code =  '$ou'
			And Dh.Batch_No =  '$batch'
	";

	$queryQT2 = oci_parse($conn_omnoi, $sql);

	oci_execute($queryQT2);

	$QT2 = oci_fetch_array($queryQT2, OCI_ASSOC+OCI_RETURN_NULLS);
